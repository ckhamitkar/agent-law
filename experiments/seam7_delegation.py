#!/usr/bin/env python3
"""
Seam 7 -- cross-hop delegation binding: the mechanism that sets c > 0.

CONTEXT (read this first)
  HORIZONTAL_LAYER.md ranks Seam 7 as spine-level and UPSTREAM of Seams 8/9/10:
  every "free-attack -> costed-attack" conversion in the doctrine spends a currency
  the identity layer never minted. exp_e_principal_cost.py then renders the bounds
  as a curve in `c` = marginal cost to mint a fresh, reputation-clean principal --
  but `c` there is a HAND-SET KNOB. Nothing in the repo actually makes a principal
  cost anything. exp_e:142 names closing that gap "the single highest-value thing
  to BUILD next." This file is that mechanism.

  It ships the three properties the doctrine needs from an identity layer:
    1. ATTENUATION -- a delegated token can only NARROW (capabilities, TTL, budget),
       never widen. Macaroon-style HMAC caveat chain: a holder attenuates WITHOUT
       the root key; the verifier detects any widening or tampering WITH it.
    2. ACCOUNTABILITY THAT SURVIVES THE CHAIN -- every token, at any delegation
       depth, is cryptographically bound to its ROOT principal. The leaf carries
       RESPONSIBILITY (the `actor`); the root keeps ACCOUNTABILITY (the
       `accountable_root`), which cannot be swapped or shed without the root key
       AND a fresh proof-of-work. This is what "cross-hop, non-fungible" means.
    3. A REAL COST -- minting a fresh root requires proof-of-work of tunable
       difficulty D, so mint_cost(D) = 2**D is an ACTUAL number of hash
       evaluations, not a literal in a sweep. This is what sets c > 0.

HONEST LIMITS (Douceur 2002, and the doctrine's own discipline)
  - This does NOT dissolve the Sybil impossibility. It INSTANTIATES one of the two
    admissible cost-sources (a real resource cost). Which cost-source to use -- PoW
    vs a trusted-authority mint vs device attestation -- is a VALUES decision, not a
    coding one. NOTE (corrected 2026-07-05): agents run SERVER-SIDE today (the Mac
    Mini, the harness, triage-backend's graph); the phone apps are local-AI clients
    with no delegation chain. So the honest principal-minter is a Mini, not a BoP
    handset -- PoW's per-device regressivity is a FUTURE constraint (activates only
    when agents go on-device), not a current one. exp_e_driven.py works the current,
    server-side numbers. The pluggable `Mint` boundary below is where the cost-source
    choice is made explicit rather than smuggled.
  - HMAC (symmetric) stands in for the production primitive (ed25519 / SPIFFE SVID).
    The verifier holds the root key. The construction and every property below port
    directly to asymmetric signatures; only the key-distribution story changes.
  - Governance incidents from this layer route to an ABSTRACT sink, never to
    triage.axionaiapps.com -- that host is the live *clinical* triage product; the
    Gemini "route to triage" framing conflated two different surfaces.
